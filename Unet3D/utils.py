...
def calculate_iou(tensor1, tensor2):
    assert tensor1.shape == tensor2.shape, "The shapes of both tensors must be the same"
    assert torch.all((tensor1 == 0) | (tensor1 == 1)), f"All elements in tensor1 must be 0 or 1. Found values: {tensor1.unique()}"
    assert torch.all((tensor2 == 0) | (tensor2 == 1)), f"All elements in tensor2 must be 0 or 1. Found values: {tensor2.unique()}"

    intersection = torch.sum(tensor1 * tensor2, dim=(1, 2))  
    union = torch.sum(tensor1, dim=(1, 2)) + torch.sum(tensor2, dim=(1, 2)) - intersection
...
class Heuri:
    ...
    def is_place_stable(self, density_map, height_map, size_after_rotate, x, y, density):
        """
        ...
        def is_qualified(support_ratio, num_support_corners, height_to):
            return (support_ratio>0.6 and num_support_corners==4 and height_to<=self.max_pallet_height) or \
                (support_ratio>0.85 and num_support_corners>=3 and height_to<=self.max_pallet_height) or \
                (support_ratio>0.9 and height_to<=self.max_pallet_height)
        ...
        if max_density_value > 0 and max_density_value < density:
            return 0
        
        density_satisfy_mask = area_density >= density
        area = height_map[x:x+length, y:y+width] * density_satisfy_mask
        vals, counts = np.unique(area, return_counts=True)
        index = np.argmax(vals)
        support_area_ratio = counts[index] / (area.shape[0] * area.shape[1])
        num_support_corners = (vals[index] == height_map[x, y] and density >= area_density[0, 0]) \
                            + (vals[index] == height_map[x+length-1, y] and density >= area_density[length-1, 0]) \
                            + (vals[index] == height_map[x, y+width-1] and density >= area_density[0, width-1]) \
                            + (vals[index] == height_map[x+length-1, y+width-1] and density >= area_density[length-1, width-1])
        height_to = vals[index] + height

        if vals[index] == 0 or is_qualified(support_area_ratio, num_support_corners, height_to):
            return 1
        else:
            return 0
    ...
    def heuri_annotation(self, pallet_obs_density, size_after_rotate, box_density):
        ...
        # 找到每个位置从顶部到最底部的第一个非零值的位置
        non_zero_mask = pallet_obs_density > 0
        non_zero_indices = np.argmax(non_zero_mask, axis=2)
        density_map = pallet_obs_density[
            np.arange(pallet_obs_density.shape[0])[:, None],
            np.arange(pallet_obs_density.shape[1]),
            non_zero_indices
        ] * (non_zero_mask.sum(axis=2) > 0)
        ...
        for x in range(int(feasible_map.shape[0]-size_after_rotate[0]+1)):
            for y in range(int(feasible_map.shape[1]-size_after_rotate[1]+1)):
                feasible_map[x, y] = self.is_place_stable(density_map, height_map, size_after_rotate, x, y, box_density)
        return feasible_map
    ...
    def __call__(self, x):
        ...
        for i in range(x.shape[0]):
            ...
            box_density = image[4][0][0][0].item()
            anno.append(self.heuri_annotation(obs_density ,box_size, box_density))
